...
so the cursor-size slider works, down to the tiny sizes some setups use. The
Windows `.cur`/`.ani` files embed 16 up to 64 px, so they stay crisp when small
and on HiDPI displays instead of scaling one image. The art starts from Dota's
32px originals, so large sizes are upscaled and look a little soft up close,
while the small ones stay crisp.
